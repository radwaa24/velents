<template>
    ...
    >
      <div class="bg-white p-6 rounded-lg w-full lg:max-w-2xl mx-10 shadow-lg relative">
        ...
        <Form @submit="handlePayment" v-slot="{ errors }">
          ...
          <p v-if="paymentStatus" class="mt-2 text-center" :class="{'text-green-500': paymentStatus === 'success', 'text-red-500': paymentStatus === 'error'}">
            {{ paymentMessage }}
          </p>
  ...
  const handlePayment = async (values) => {
    ...
    try {
      // Call Stripe's confirmCardPayment
